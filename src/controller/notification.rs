...
#[post("/unsubsribe/<product_type>?<url>")]
pub fn unsubsribe(product_type: &str, url: &str) -> Result<Json<Subscriber>> {
    return match NotificationService::unsubsribe(product_type, url) {
        Ok(f) => Ok(Json::from(f)),
        Err(e) => Err(e)
    };
}